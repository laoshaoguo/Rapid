...
/**
 * @Author Jliuer
 * @Date 2017/2/16/16:08
 * @Email Jliuer@aliyun.com
 * @Description 仅仅适用与沾满一个屏幕时，依据屏幕坐标中心点判断view
 */
public class RecyclerViewUtils {
    ...
    public static boolean isChildInCenterX(RecyclerView recyclerView, View view) {
        ...
        if (childCount > 0) {
            ...
            if (vLocationOnScreen[0] <= middleX && vLocationOnScreen[0] + view.getWidth() >=
                    middleX) {
                return true;
            }
        }
    ...
    public static boolean isChildInCenterY(RecyclerView recyclerView, View view) {
        ...
        if (childCount > 0) {
            view.getLocationOnScreen(vLocationOnScreen);
            if (vLocationOnScreen[1] <= middleY && vLocationOnScreen[1] + view.getHeight() >=
                    middleY) {
                return true;
            }
        }
